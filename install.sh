...
install_pkgs_src(){
  logo "Install source packages"
  if command -v pacman &>/dev/null; then
    pacman -Sy --noconfirm base-devel atk cairo gtk3 gsl libx11 libxpm libxml2
  elif command -v apt &>/dev/null; then
    apt install -y build-essential libx11-dev libxpm-dev libxt-dev libxext-dev pkg-config libxml2-dev libgtk-3-dev libxinerama-dev libxtst-dev libgsl-dev
  elif command -v dnf &>/dev/null; then
    dnf groupinstall -y "C Development Tools and Libraries" "Development Tools"
    dnf install -y atk cairo gtk3 gsl libX11 libXpm libxml2
  elif command -v zypper &>/dev/null; then
    zypper in -t -y pattern devel_basis
  elif command -v xbps &>/dev/null; then
    xbps-install -Sy base-devel atk cairo gsl libX11 libXpm libxml2
  fi
  curl https://www.ratrabbit.nl/downloads/xsnow/xsnow-3.7.6.tar.gz -o /tmp/xsnow.tar.gz
  tar -xvf /tmp/xsnow.tar.gz /tmp
  cd /tmp/xsnow-3.7.6 || return
  ./configure
  make
  make install
  cd ..
...
clone_dotfiles(){
    ...
    [ -d /tmp/christmas-bspwm-dotfiles ] && rm -rf /tmp/christmas-bspwm-dotfiles
    git clone --depth=1 https://github.com/p3nguin-kun/unixporn-dotfiles -b christmas /tmp/christmas-bspwm-dotfiles
    done_msg
}
...
cp -R /tmp/christmas-bspwm-dotfiles/config/* /home/$username/.config
cp -R /tmp/christmas-bspwm-dotfiles/themes/* /home/$username/.themes
cp -R /tmp/christmas-bspwm-dotfiles/fonts/* /home/$username/.fonts
cp -RT /tmp/christmas-bspwm-dotfiles/home/ /home/$username
...
config_firefox(){
  ...
  sleep 2
  cp -R /tmp/christmas-bspwm-dotfiles/misc/firefox/* /home/$username/.mozilla/firefox/*.default-*/
